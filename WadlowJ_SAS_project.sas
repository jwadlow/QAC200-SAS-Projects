...
DATE: Thursday, January 08, 2015     TIME: 9:14:01 AM
PROJECT: WadlowJ_SAS_project_7DEC15
PROJECT PATH: \\Client\P$\QAC\qac200\students\jwadlow\Assignments\WadlowJ_SAS_project_7DEC15.egp
---------------------------------------- */
...
%LET _CLIENTPROJECTPATH='\\Client\P$\QAC\qac200\students\jwadlow\Assignments\WadlowJ_SAS_project_7DEC15.egp';
%LET _CLIENTPROJECTNAME='WadlowJ_SAS_project_7DEC15.egp';
...
/* -------------------------------------------------------------------
   ...
   Generated on: Thursday, January 08, 2015 at 9:12:32 AM
...
GOPTIONS ACCESSIBLE;
%_eg_conditional_dropds(MYDATA.'MEPS Variable Filter Results'n);

PROC SQL;
   CREATE TABLE MYDATA.'MEPS Variable Filter Results'n(label="MEPS Variable Filter Results") AS 
   SELECT t1.DUPERSID, 
          t1.AGE12X, 
          t1.REGION12, 
          t1.RACETHX, 
...
/* -------------------------------------------------------------------
   ...
   Input Data: Local:MYDATA.MEPS VARIABLE FILTER RESULTS
   Server:  Local
   ------------------------------------------------------------------- */

%_eg_conditional_dropds(WORK."%STR(CONTContentsForMEPS VARIABLE FIL)"n);
TITLE;
FOOTNOTE;
FOOTNOTE1 "Generated by the SAS System (&_SASSERVERNAME, &SYSSCPL) on %TRIM(%QSYSFUNC(DATE(), NLDATE20.)) at %TRIM(%SYSFUNC(TIME(), TIMEAMPM12.))";
PROC FORMAT;
...
PROC DATASETS NOLIST NODETAILS; 
   CONTENTS DATA=MYDATA.'MEPS VARIABLE FILTER RESULTS'n OUT=WORK.SUCOUT1;

RUN;

DATA WORK.'CONTContentsForMEPS VARIABLE FIL'n(LABEL="Contents Details for MEPS VARIABLE FILTER RESULTS");
   SET WORK.SUCOUT1;
RUN;
...
CREATE VIEW WORK.SCVIEW AS 
	SELECT DISTINCT memname LABEL="Table Name", 
			...
			charset LABEL="Char. Set", 
			protect LABEL="Password Protected", 
			typemem LABEL="Data Set Type" FROM WORK.'CONTContentsForMEPS VARIABLE FIL'n
	ORDER BY memname ; 

CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.SCVIEW
		WHERE memname='MEPS VARIABLE FILTER RESULTS';
QUIT;
...
PROC SORT DATA=WORK.'CONTContentsForMEPS VARIABLE FIL'n OUT=WORK.'CONTContentsForMEPS VARIABLE FIL'n;
   BY memname name;
RUN;
...
DROP TABLE WORK.SCTABLE;
CREATE TABLE WORK.SCTABLE AS
	SELECT * FROM WORK.'CONTContentsForMEPS VARIABLE FIL'n
		WHERE memname='MEPS VARIABLE FILTER RESULTS';
QUIT;
